...
class SystemController extends BaseController
{
    ...
    public function saveHospitalEmailSettings(Request $request)
    {
    ...
    public function saveHospitalSettings(Request $request)
    {
        ...
        if (\Auth::user()->can('manage hospital settings')) {
            $user = \Auth::user();
            $request->validate(
                [
    ...
    public function saveBusinessSettings(Request $request)
    {
        ...
        if (\Auth::user()->can('manage business settings')) {
            ...
            if ($request->hospital_logo_dark) {
                ...
                $dir = 'uploads/logo';
                $path = Utility::upload_file($request, 'hospital_logo_dark', $logoName, $dir, $validation);
                if ($path['flag'] == 1) {
                    $logo = $path['url'];
                } else {
            ...
            if ($request->hospital_logo_light) {
        ...
        //                $logoName = 'logo-light.png';
                ...
                $dir = 'uploads/logo';
                $path = Utility::upload_file($request, 'hospital_logo_light', $logoName, $dir, $validation);
                if ($path['flag'] == 1) {
                    $logo = $path['url'];
                } else {
                    return redirect()->back()->with('error', __($path['msg']));
        ...
        //                $favicon = 'favicon.png';
                ...
                $dir = 'uploads/logo/';
                $path = Utility::upload_file($request, 'hospital_favicon', $favicon, $dir, $validation);
                if ($path['flag'] == 1) {
                } else {
                    return redirect()->back()->with('error', __($path['msg']));
                }
    ...
    public function hospitalIndex(Request $request)
    {
        if (\Auth::user()->can('manage hospital settings')) {
            ...
            $hospital_payment_setting = Utility::getHospitalPaymentSetting(\Auth::user()->creatorId());
            ...
            return view('settings.hospital', compact('setting', 'hospital_payment_setting', 'timezones',
    ...
    public function saveHospitalPaymentSettings(Request $request)
    {
        ...
        foreach ($post as $key => $data) {
            ...
            \DB::insert(
                'insert into hospital_payment_settings (`value`, `name`,`created_by`) values (?, ?, ?) ON DUPLICATE KEY UPDATE `value` = VALUES(`value`) ', $arr
            );
    ...
    {
        if (\Auth::user()->type == 'hospital' || \Auth::user()->type == 'super admin') {
            $validator = \Validator::make(
                $request->all(),
                [
                    'ip' => 'required',
