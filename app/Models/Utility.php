...
class Utility extends Model
{
    ...
    public static function settings()
    {
        ...
        $settings = [
            ...
            "hospital_logo_light" => "logo-light.png",
            "hospital_logo_dark" => "logo-dark.png",
            "hospital_favicon" => "favicon.png",
            "cust_theme_bg" => "on",
            "cust_darklayout" => "off",
            "color" => "",
            "SITE_RTL" => "off",
...
//        return ($taxRate / 100) * (($price-$discount) * $quantity);
    ...
    public static function chartOfAccountTypeData($hospital_id)
    {
...
// chart of account for new hospital
    ...
    public static function sendEmailTemplate($emailTemplate, $mailTo, $obj)
    {
        ...
        if ($usr->type != 'Super Admin') {
            ...
            if (isset($template) && !empty($template)) {
                // check template is active or not by hospital
                if ($usr->type != 'super admin') {
    ...
    public static function replaceVariable($content, $obj)
    {
        ...
        $arrValue['app_name'] = !empty($hospital_name) ? $hospital_name : env('APP_NAME');
        $arrValue['hospital_name'] = self::settings()['mail_from_name'];
...
//    }
    ...
    public static function hospitalData($hospital_id, $string)
    {
        $setting = DB::table('settings')->where('created_by', $hospital_id)->where('name', $string)->first();
        if (!empty($setting)) {
    ...
    public static function addNewData()
    {
        ...
        $hospitalRole = Role::where('name', 'LIKE', 'hospital')->first();

        $hospitalPermissions = $hospitalRole->getPermissionNames()->toArray();
        $hospitalNewPermission = [
            'manage form builder',
            'create form builder',
            'edit form builder',
            'delete form builder',
        ...
        foreach ($hospitalNewPermission as $op) {
            // check if permission is not assign to owner then assign.
            if (!in_array($op, $hospitalPermissions)) {
                $permission = Permission::findByName($op);
                $hospitalRole->givePermissionTo($permission);
            }
        }
    ...
    public static function getHospitalPaymentSetting($user_id)
    {

        $data = \DB::table('hospital_payment_settings');
        $settings = [];
        $data = $data->where('created_by', '=', $user_id);
        $data = $data->get();
    ...
    public static function getHospitalPayment()
    {
    ...
    public static function GetLogo()
    {
        ...
        if (\Auth::user() && \Auth::user()->type != 'super admin') {
            ...
            if (Utility::getValByName('cust_darklayout') == 'on') {
                ...
                return Utility::getValByName('hospital_logo_light');
            } else {
                return Utility::getValByName('hospital_logo_dark');
            }
        } else {
            if (Utility::getValByName('cust_darklayout') == 'on') {
...
//                    dd(\Storage::disk(),$path);
    ...
    public static function updateStorageLimit($hospital_id, $image_size)
    {
        $image_size = number_format($image_size / 1048576, 2);

        $user = User::find($hospital_id);
    ...
    public static function changeStorageLimit($hospital_id, $file_path)
    {
    ...
    public static function referralTransaction($plan , $hospital= '')
    {
        if($hospital != '')
        {
            $objUser = $hospital;
        ...
        $user = ReferralTransaction::where('hospital_id' , $objUser->id)->first();
        ...
        if($objUser->used_referral_code != 0 && $user == null && (isset($referralSetting) && $referralSetting->is_enable == 1))
        {
            ...
            $transaction->hospital_id = $objUser->id;
